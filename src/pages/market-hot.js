...
function renderConceptRank() {
  return `
    <div class="mb-4 flex items-center gap-2">
      <i data-lucide="flame" style="width:18px; height:18px; color:var(--state-error);"></i>
      <span style="font-size:var(--text-body); color:var(--ink-3);">按涨幅排序 · 数据仅供参考</span>
    </div>
...
export function createMarketHotPage(root) {
  ...
  function render() {
    root.innerHTML = `
      ...
      ${activeTab === 'rank' ? renderConceptRank() : ''}
      ${activeTab === 'finance' ? renderFinanceNews() : ''}
      ${activeTab === 'stock' ? renderStockNews() : ''}
      ...
      <div style="background:var(--surface-2); border-left:3px solid var(--brand); border-radius:0 var(--r-md) var(--r-md) 0; padding:var(--s-4) var(--s-5); margin-top:var(--s-6);">
        ...
        <p style="font-size:var(--text-body); line-height:var(--leading-body); color:var(--ink-2);">当前展示为示例数据。后期可接入实时行情API（如东方财富、同花顺等），自动抓取概念板块排行、财经新闻和个股资讯，实现数据实时更新。</p>
      </div>
    `
    refreshIcons()
    bindEvents()
